...
结果就可能不是你预期的结果。其他属于 Eq 类的类型：
    ...
    -- 如果类型 a 属于 Eq 类，那么 Maybe a 才属于 Eq 类
    instance Eq a => Eq [a]
...
### 6. Monad 类(单子)
    ...
    class Monad m where
        -- returnM
        return :: a -> m a
        -- bindM, 注意第二个参数是一个函数，返回单子
        (>>=) :: m a -> (a -> m b) -> m b
        -- thenM
        (>>) :: m a -> m b -> m b
        -- ma.bindM(fn) <=>  bindM(ma,fn)
...
对于 List: 
    ...
    x.bindM(fn).show();  // [101,102,103,201,202,303]
    // 或者直接写为
...
我前面说过，Maybe可以看作是长度为 0 或 1 的字符串。如果更长的话写
...
`[101,201,301,102,202,302,301,302,303]`,其实如果写成这样（目前 jHaskell 还不支持）
...
g 和 f 都是一个函数,g.f 就是函数复合的意思 h = g.f，就是说对于任意参数 h(x) = g(f(x))。
...
比较麻烦（要写括号），目前在 jHaskell 中大部分函数还不是curry化的函数。`fmap id
